...
import kr.co.bepo.cameraapp.adapter.ImageViewPagerAdapter
import kr.co.bepo.cameraapp.databinding.ActivityImageListBinding
import kr.co.bepo.cameraapp.util.PathUtil
import java.io.File
...
class ImageListActivity : AppCompatActivity() {
    ...
    override fun onDestroy() {
        setResult(Activity.RESULT_OK, Intent().apply {
            putExtra(URI_LIST_KEY, putExtra(URI_LIST_KEY, ArrayList<Uri>().apply { imageViewPagerAdapter.uriList.forEach { add(it) } }))
        })
        super.onDestroy()
    }
}